...
**Key property**: Tactics are **independent** — applying `Z3` after `model_parallelism` produces the same result regardless of order (though the resulting communication pattern differs). Each tactic is applied sequentially; the compiler propagates shardings and inserts collectives after each tactic, making the result verifiable at each step.
